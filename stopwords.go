...
func (stop *StopWords) stopWordsCount(config *Configuration, lang string, text string, items []string) wordStats {
	if len(text) == 0 && len(items) == 0 {
		return wordStats{}
	}
	if len(items) == 0 {
		items = config.Words(text)
	}
	if len(items) == 0 {
		return wordStats{}
	}
...
func (stop StopWords) SimpleLanguageDetector(config *Configuration, text string) string {
	max := 0
	currentLang := "en"

	items := config.Words(text)
	for k := range sw {
		ws := stop.stopWordsCount(config, k, text, items)
